...
Texture2D<float4> InputTex : register(s0);
...
void main(uint3 gruoupId : SV_GroupID, uint3 groupThredId : SV_GroupThreadID, uint groupIndex : SV_GroupIndex, uint3 dispatchId : SV_DispatchThreadID)
{
    ...
    float3 result = blurWeights[radius] * InputTex[pixelIndex].rgb;
    ...
    for (int i = -radius; i <= radius; ++i)
    {
        ...
        result += weight * InputTex[mad(i, direction2, pixelIndex)].rgb;
        totalWeight += weight;
    }
    
    OutputTex[pixelIndex] = float4(result / totalWeight, 1);
}