...
void GameState::loadModels()
{
	...
	//this->m_models.emplace_back();
	//mat.diffuse = DirectX::XMFLOAT4(1.f, 1.f, 1.f, 1.f);
	//this->m_models[33].initializeModelBff(m_device, m_dContext, "Bell1.bff", mat, L"Textures/ColorTexture.png"); //load model

	//33. Bell 2
	this->m_models.emplace_back();
	mat.diffuse = DirectX::XMFLOAT4(1.f, 1.f, 1.f, 1.f);
	this->m_models[33].initializeModelBff(m_device, m_dContext, "Bell2.bff", mat, L"Textures/ColorTexture.png"); //load model

	////35. Bell 3
	//this->m_models.emplace_back();
	//mat.diffuse = DirectX::XMFLOAT4(1.f, 1.f, 1.f, 1.f);
	//this->m_models[35].initializeModelBff(m_device, m_dContext, "Bell3.bff", mat, L"Textures/ColorTexture.png"); //load model
}
...
void GameState::initlialize(ID3D11Device* device, ID3D11DeviceContext* dContext, GameOptions options, std::shared_ptr<DirectX::AudioEngine> audioEngine)
{
	...
	for (size_t i = 0; i < NR_OF_CHAIN_LINKS; i++)
	{
		...
		this->addGameObjectToWorld(true, false, 1, 5, &m_models[5], vec, XMVectorSet(.9f, .9f, .9f, 1.f), XMFLOAT3(1.f, 1.f, 1.f), XMFLOAT3(2.f, 2.f, 2.f));
